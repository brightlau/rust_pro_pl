 ...
 *  代码生成方式	    编译期展开（生成具体代码）	            运行时调用
 *  错误提示	        格式不匹配时给出精确错误位置	        仅类型不匹配错误
 *  trait 自动应用	    自动调用 Display/Debug trait	      需要手动指定 trait 实现
 * 
 * 为什么 Rust 使用宏而不是函数
 * 
 * 1. 可变参数需求：处理不定数量的格式化参数
 *  println!("Values: {}, {}, {}", 1, "two", 3.14); // ✅
 * 2. 编译时格式验证：
 *  println!("Missing: {}");  // ❌ 编译时错误：1 个占位符但提供了 0 个参数
 * 3. 复杂占位符支持：
 *  println!("Hex: {:x}, Binary: {:b}", 255, 15); // 十六进制和二进制输出
 * 4. 性能优化：
 *  ::std::io::_print(::core::fmt::Arguments::new_v1(&["Hello"], &[]));  // 宏展开后
 * 
 * 宏的独特优势：
 * 
 * 1. 编译时展开：会在编译阶段将宏展开为具体代码，通过 cargo expand 查看展开结果
 * 2. 元编程能力：可以生成重复代码模式，例如自动生成测试用例避免重复编写测试模板代码
 * 3. 语法扩展：支持自定义语法结构，如 vec![] 或 路由宏
 * 4. 性能优化：在编译期完成格式检查，运行时无额外开销，可以比较宏和函数的运行时行为，
 * 比如宏生成的代码直接内联，避免了函数调用的开销，或者编译时检查避免了运行时的错误处理
 * 
 * 宏工作机制解析：
 * 
 * 1. 编译阶段展开:
